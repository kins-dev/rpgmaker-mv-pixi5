...
ImageCache.prototype._mustBeHeld = function (item) {
    // request only is weak so it can be purged
...
Graphics._onKeyDown = function (event) {
    if (!event.ctrlKey && !event.altKey) {
        // Replace with key and update codes to strings
...
Sprite.prototype._renderCanvas_PIXI = PIXI.picture.Sprite.prototype._renderCanvas;
Sprite.prototype._render_PIXI = PIXI.picture.Sprite.prototype._render;
...
Sprite.prototype._speedUpCustomBlendModes = function (renderer) {
    ...
    if (renderer.renderingToScreen && renderer.renderTexture.current) {
 ...
 * @private
 */
// May need to split sprites from picture sprites
...
Tilemap.prototype._paintTiles = function (startX, startY, x, y) {
    ...
    const lowerTiles = [];
    const upperTiles = [];
    const lowerLayer = this._lowerBitmap;
    const upperLayer = this._upperBitmap;
...
TilingSprite.prototype = Object.create(PIXI.picture.TilingSprite.prototype);
...
TilingSprite.prototype._renderCanvas_PIXI = PIXI.picture.TilingSprite.prototype._renderCanvas;
TilingSprite.prototype._render_PIXI = PIXI.picture.TilingSprite.prototype._render;
...
TilingSprite.prototype.updateTransformTS = PIXI.picture.TilingSprite.prototype.updateTransform;
...
// ??? May not be needed
Window.prototype._renderCanvas_PIXI = PIXI.Container.prototype._renderCanvas;
Window.prototype._render_PIXI = PIXI.Container.prototype._render;
 ...
 * @private
 */
Window.prototype._renderCanvas = function (renderer) {
    /*
    if (this.contents.bitmap && !this.contents.bitmap.isReady()) {
        return;
    }*/

    if (this.texture.frame.width > 0 && this.texture.frame.height > 0) {
        if (this.contents.bitmap) {
            this.contents.bitmap.touch();
            this.contents.bitmap.checkDirty();
        }
    }
    this._renderCanvas_PIXI(renderer);
};
 ...
 * @private
 */
Window.prototype._render = function (renderer) {

    if (this.contents && !this.contents.isReady()) {
        return;
    }
    if (this.contents) {
        this.contents.checkDirty();
    }
    if (this.contents) {
        this.contents.touch();
    }
...
WindowLayer.prototype.initialize = function () {
    ...
    */
    // Removed as this caused problems
...
WindowLayer.prototype._render_PIXI = PIXI.Container.prototype.render;
...
WindowLayer.prototype.render = function (renderer) {
    ...
    // Masking doesn't appear to be needed anymore, but
    // should try putting it back in
    ...
    // Limit flushing as much as possible, but need this for
    // the loading screen to show up correctly
    // but this is also causing white flashes
    //if (item_drawn) {
...
console.log(inspect(CS_URL.urlMap));
